...
class FourbarDesign():
    ...
    def WriteTorqueFactorFile(self, nframes=101, filename = "TorqueFactor.txt"):

        if len(self.forcepoints) == 0: #no forcepoint was given in the file
            return  #so leave!

        fb=self.fourbar
        th2 = np.array([])
        locations = np.zeros( (nframes, len(self.forcepoints) * 2) )
        derivatives = np.zeros_like(locations)
        ...
        for frame in range(0,nframes):
            ...
            # move and rotate  ..  this point array               translate to    rotate to             rotate about
            points = []
            for point in self.forcepoints:
                points.append( [ point[1], point[2] ] )
            points = transform(points, fb.ax, fb.ay, (fb.th3 - fb.th3start), fb.axstart, fb.aystart)
            for i in range(len(points)):
                locations[frame][i*2] = points[i][0]
                locations[frame][i*2 + 1] = points[i][1]

        #next frame

        deltath2 = (fb.theta2end-fb.theta2start)/(nframes-1)
        dlocationdth2 = np.gradient(locations,edge_order=2)[0]/deltath2

        f1 = open(filename,"w")
        line = "        Torque Factors (Estimated using Numerical Derivatives) \n"
        f1.write(line)

        line = "** Motor Torque = Summation ( force applied at each point * Torque Factor at each point) **\n\n"
        f1.write(line)

        line = "                      Force Point Locations "
        f1.write(line)
        for point in self.forcepoints:
            line = "\n{:15s}   x = {:10.3f}   y = {:10.3f} ".format(point[0],point[1],point[2])
            f1.write(line)

        line = "\n\n                        "
        for point in self.forcepoints:
            line += "{:^22}".format(point[0])
        f1.write(line)
        line = "\nDriving Link Angle (deg)"
        for point in self.forcepoints:
            line +=  "     X          Y     "
        f1.write(line)
        for i in range(len(th2)):
            line = "\n{:15.3f}".format(th2[i] *180/np.pi) + "       "
            for j in range(len(self.forcepoints)):
                val = dlocationdth2[i, j*2]
                line += "{:11.3f}".format(val)
                val = dlocationdth2[i, j*2+1]
                line += "{:11.3f}".format(val)
            f1.write(line)

        f1.close()
        fb.SetToStartingPosition()

    def CalculateTracepoints(self):
        if len(self.tracepoints) == 0: #no forcepoint was given in the file
            return  #so leave!

        npoints = 360
        fb=self.fourbar
        self.tracepointLocations = np.zeros( (npoints, len(self.tracepoints) * 2) )
        self.tracepointJoints = np.zeros( (npoints, 4))
        th2 = []

        for row in range(0,npoints):
            if row == 0: #use the original theta data
                fb.th2 = fb.theta2start
                fb.th3 = fb.th3start
                fb.th4 = fb.th4start
            fb.th2 = fb.theta2start + (fb.theta2end-fb.theta2start) * row/(npoints-1)
            th2 = np.append(th2, fb.th2)
            ...
            self.tracepointJoints[row][0]=fb.ax
            self.tracepointJoints[row][1]=fb.ay
            self.tracepointJoints[row][2]=fb.bx
            self.tracepointJoints[row][3]=fb.by
            ...
            for j in range(len(points)):
                self.tracepointLocations[row][j*2] = points[j][0]
                self.tracepointLocations[row][j*2 + 1] = points[j][1]
        self.tracepointAngles = th2
    ...
    def WriteTracepointsFile(self, filename = "Tracepoints.txt"):

        if len(self.tracepoints) == 0: #no forcepoint was given in the file
            return  #so leave!

        self.CalculateTracepoints()
        ...
        fb=self.fourbar
        th2 = self.tracepointAngles
        locations = self.tracepointLocations
        jointLocations = self.tracepointJoints

        line = "                      Tracepoint Locations "
        f1.write(line)
        for point in self.tracepoints:
            line = "\n{:15s}   x = {:10.3f}   y = {:10.3f} ".format(point[0],point[1],point[2])
            f1.write(line)
        ...
        for point in self.tracepoints:
            line += "{:^22}".format(point[0])
        f1.write(line)
        line = "\nInput (deg)          ax        ay          bx         by   "
        for point in self.tracepoints:
            line +=  "     X          Y     "
        f1.write(line)
        for i in range(len(th2)):
            line = "\n{:6.3f},".format(th2[i] *180/np.pi) + "       "

            val = jointLocations[i, 0]  # write the joint locations
            line += "{:11.3f}".format(val)
            val = jointLocations[i, 1]
            line += ",{:11.3f}".format(val)
            val = jointLocations[i, 2]
            line += ",{:11.3f}".format(val)
            val = jointLocations[i, 3]
            line += ",{:11.3f}".format(val)

            for j in range(len(self.tracepoints)):
                val = locations[i, j*2]
                line += ",{:11.3f}".format(val)
                val = locations[i, j*2+1]
                line += ",{:11.3f}".format(val)
            f1.write(line)

        f1.close()
        self.SetToStartingPosition()



    def CreateDraggingList(self):
        # put locations of "draggable" things into a  list
        draglist = [[self.fourbar.ax, self.fourbar.ay],
                    [self.fourbar.bx, self.fourbar.by]]
        return draglist
    ...
    def DraggingListItemChanged(self, x, y, draglist, index):
        ...
        draglist[index]=[x,y]
        if index == 0:  # point a
            (self.fourbar.ax, self.fourbar.ay) = (x,y)
        if index == 1:  # point b0
            (self.fourbar.bx, self.fourbar.by) = (x,y)
        self.DesignFourbar()
